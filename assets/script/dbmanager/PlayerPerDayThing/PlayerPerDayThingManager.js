...
var PlayerPerDayThingManager = app.BaseClass.extend({
    ...
    SetPlayerPerDayThingProperty:function(eventID, property, value){
        ...
        if(!dataInfo){
                            console.error("SetPlayerPerDayThingProperty(%s,%s,%s) not find eventID", eventID, property, value);
            return 
        }
        if(!dataInfo.hasOwnProperty(property)){
                            console.error("SetPlayerPerDayThingProperty(%s,%s,%s) property not find", eventID, property, value);
            return
        }
    ...
    OnPack_PerDayThingProperty:function(pack){
        ...
        if(!this.PlayerPerDayThingIndexToProperty.hasOwnProperty(index)){
                            console.error("OnPerDayThingProperty has not index{1}",index)
            return
        }
    ...
    GetPlayerPerDayThingInfo:function(eventID){
        let dataInfo = this.dataInfo[eventID];
        if(!dataInfo){
                            console.error("GetPlayerPerDayThingInfo not find:%s", eventID);
            return
        }
        return dataInfo
    },
    ...
    GetPlayerPerDayThingProperty:function(eventID, property){
        ...
        let dataInfo = this.dataInfo[eventID];
        if(!dataInfo){
                            console.error("GetPlayerPerDayThingProperty(%s,%s) not find key", eventID, property);
            return 
        }
        if(!dataInfo.hasOwnProperty(property)){
                            console.error("GetPlayerPerDayThingProperty(%s,%s) property not find", eventID, property);
            return
        }
