...
var PlayerHelpManager = app.BaseClass.extend({
        ...
        for(let index=0; index<count; index++){
            ...
            if(eventClickID){
                if(!this.EventClickInfo[eventClickID]){
                                    console.error("eventID(%s) EventClickInfo.txt (%s) not find", eventID, eventClickID);
                    continue
                }
            }
	        if(eventTalkIDList){
		        ...
		        for(let talkIndex=0; talkIndex<talkCount; talkIndex++){
			        ...
			        if(!this.EventTalkInfo.hasOwnProperty(talkID)){
				                        console.error("EventTalkInfo.txt not find:%s", talkID);
			        }
		        }
	        }
            ...
            if(this.DBEventIDList.InArray(eventID)){
                                console.error("InitData DBEventIDList have find:%s", eventID);
                continue
            }
    ...
    RunEvent:function(eventID){
        ...
        let eventInfo = this.HelpEvent[eventID];
        if(!eventInfo){
                            console.error("RunEvent(%s) HelpEvent.txt not find", eventID);
            return
        }
        ...
        if(eventTalkIDList.length){
	        ...
	        if(sceneType == "bossScene"){
		        ...
		        app.FightControl_Tower().EventTalkPauseTowerFight();
		                        ...
		                        console.error("RunEvent sceneType:%s error", sceneType);
	        }
        }
    ...
    TriggerNextEvent:function(oldEventID){
        ...
        if(!eventInfo){
                            console.error("TriggerNextEvent(%s) HelpEvent.txt not find", oldEventID);
            return
        }
        ...
        if(!nextEventInfo){
                            console.error("TriggerNextEvent HelpEvent.txt (%s) not find", nextEventID);
            return
        }
    ...
    OnClickEventButton:function(eventID){
        ...
        if(eventID != this.waitEventID){
                            console.error("OnClickEventButton(%s) != (%s)(%s)", eventID, this.waitEventID, this.eventID);
            return
        }
    ...
    OnTalkEnd:function(eventID){
	    ...
	    if(sceneType == "bossScene"){
		    ...
		    app.FightControl_Tower().EndEventTalkContinueTowerFight();
		                    ...
		                    console.error("OnTalkEnd sceneType:%s error", sceneType);
	    }

        if(eventID != this.waitEventID){
                            console.error("OnTalkEnd(%s) != (%s)(%s)", eventID, this.waitEventID, this.eventID);
            return
        }
        let eventInfo = this.HelpEvent[this.waitEventID];
    ...
    OnFormShow:function(formName){
        ...
        if(!helpEvent){
                            console.error("OnFormShow HelpEvent.txt not find:%s", this.waitEventID);
            return
        }
        ...
        if(!eventClickInfo){
                            console.error("(%s) OnFormShow not find(%s)", this.waitEventID, eventClickID);
            return
        }
    ...
    GetClickEventNode:function(clickID){
        ...
        if(!eventClickInfo){
                            console.error("GetClickEventNode(%s) not find", clickID);
            return 
        }
        ...
        if(childFormName){
            ...
            if(childCount != 1){
                                console.error("GetClickEventNode(%s) childFormName:%s,:%s error", clickID, childFormName, childCount);
                return
            }
        ...
        if(viewChildFormName){
            ...
            if(!formComponent){
                                console.error("GetClickEventNode(%s) viewChildFormName:%s,:%s error", clickID, viewChildFormName, childIndex)
                return
            }
        }
        ...
        if(!eventButtonNode){
                            console.error("GetClickEventNode(%s) not find (%s) node", formName, buttonPath);
            return
        }
        let button = eventButtonNode.getComponent(cc.Button);
        if(!button){
                            console.error("GetClickEventNode(%s,%s) not find cc.Button", formName, buttonPath);
            return
        }
