...
cc.Class({
    ...
    ShowHelp:function(eventID, eventNode){
	    if(!this.GuideLayerNodeComponent){
		                    console.error("ShowHelp(%s) error", eventID);
		    return
	    }
    ...
    ShowMap:function(){
	    ...
	    // if(!this.BackgroundLayerComponent){
		   //                  console.error("ShowMap not find BackgroundLayerComponent");
		   //  return
	    // }
    ...
    GetModelParent:function(imageName, modelRes, userData){
	    ...
	    if(!this.ModelLayerNode){
		                    console.error("GetModelParent(%s) not ModelLayerNode", imageName)
		    return
	    }
        //如果创建的是没有贴图的模型
