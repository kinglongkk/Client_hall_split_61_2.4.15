...
var PokerCard = app.BaseClass.extend({
    ...
    LoadAllPokerRes: function () {
        ...
        if (0 != this.miniPokerList.length)
            return;
        cc.resources.loadDir("texture/game/dhd/mini", cc.SpriteFrame, function (err, assets) {
            if (err) {
                cc.error(err);
                return;
            }
        ...
        cc.resources.loadDir("texture/aycp/pokercard", cc.SpriteFrame, function (err, assets) {
            if (err) {
                cc.error(err);
                return;
            }
