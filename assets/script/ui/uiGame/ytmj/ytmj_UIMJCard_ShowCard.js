...
cc.Class({
	...
	ShowAllDownCard:function(){
		let room = this.RoomMgr.GetEnterRoom();
		if(!room){
			                console.error("ShowAllDownCard not enter room");
			return
		}
		//UICard01
		...
		if(!setPos){
			                console.error("ShowAllDownCard(%s) not find:%s", nodeParentName, pos);
			return
		}
	...
	ShowDownImg:function(cardIDList,handCard=0,imageString,jin1=0,jin2=0){
		...
		for(let index=0; index<count; index++){
			...
			let childNode = this.node.getChildByName(childName);
			if(!childNode){
				                console.error("ShowAllDownCard not find childName:%s", childName);
				continue
			}
	...
	ShowImage:function(childNode, imageString, cardID){
		...
		if(!imageInfo){
			                console.error("fuck ShowImage IntegrateImage.txt not find:%s", imageName);
			return
		}
		//图片没有变化
