...
cc.Class({
	...
	HideAllChild: function () {
		for (let index = 1; index <= this.ChildCount; index++) {
			...
			for (let indexChild = 1; indexChild <= this.PaiChildCount; indexChild++) {
				...
				if (!paiNode) {
					                console.error("HideAllChild(%s) not find:%s", childName, paiChildName);
					continue
				}
	...
	ShowTipOutCard: function (cardID) {
		...
		for (let index = 1; index <= this.ChildCount; index++) {
			...
			let childNode = this.node.getChildByName(childName);
			if (!childNode) {
				                console.error("ShowTipOutCard not find:%s", childName);
				continue
			}
	...
	ShowAllOutEatCard: function () {
		let room = this.RoomMgr.GetEnterRoom();
		if (!room) {
			                console.error("ShowAllOutEatCard not enter room");
			return
		}
		//UICard01
		...
		if (!setPos) {
			                console.error("ShowEatCard(%s) not find:%s", nodeParentName, pos);
			return
		}
		...
		for (let index = 0; index < count; index++) {
			...
			let childNode = this.node.getChildByName(childName);
			if (!childNode) {
				                console.error("ShowEatCard not find childName:%s", childName);
				continue
			}
			childNode.active = true;
			let cardCount = cardIDList.length;
			for (let cardIndex = 0; cardIndex < cardCount; cardIndex++) {
				...
				let childNode = cc.find(childPath, this.node);
				if (!childNode) {
					                console.error("ShowEatCard not find childPath:%s", childPath);
					continue
				}
			...
			for (let cardIndex = cardCount + 1; cardIndex <= this.PaiChildCount; cardIndex++) {
				...
				let childNode = cc.find(childPath, this.node);
				if (!childNode) {
					                console.error("ShowEatCard not find:%s", childPath);
					continue
				}
				let cardSprite = childNode.getComponent(cc.Sprite);
				cardSprite.spriteFrame = null;
	...
	ShowImage: function (childNode, imageString, cardID) {
		...
		let imageInfo = this.IntegrateImage[imageName];
		if (!imageInfo) {
			                console.error("ShowImage IntegrateImage.txt not find:%s", imageString);
			return
		}
		let imagePath = imageInfo["FilePath"];
		let that = this;
