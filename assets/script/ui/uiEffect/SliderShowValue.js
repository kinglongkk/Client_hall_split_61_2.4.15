...
cc.Class({
    ...
    SetWndImage:function(wndNode, value){
        ...
        if(!sprite){
                            console.error("SetWndImage(%s,%s) not find cc.Sprite", wndNode.name, value);
            return
        }
    ...
    GetSpriteFrameByImageName:function(imageName){
        ...
        if(!imageInfo){
                            console.error("GetSpriteFrameByImageName IntegrateImage.txt not find key(%s)", imageName);
        ...
        if(filePath.startsWith("atlas")){
            ...
            if(!fileName){
                                console.error("GetSpriteFrameByImageName(%s) use atlas(%s) need appoint FileName", imageName, filePath);
    ...
    GetShowValue:function(){
	    if(!this.NowLabel){
		                    console.error("GetShowValue not NowLabel");
		    return 0
	    }
