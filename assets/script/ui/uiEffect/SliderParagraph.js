...
cc.Class({
		...
		for(let index in this.paragraphPositionDict){
			...
			if(paragraphPositionDict["KeyID"] == keyID){
				...
				return
		                ...
		                console.error("SetChooseKeyID(%s) not find", keyID);
	},
	...
	GetProgressNodeByKeyID:function(keyID){
		...
		if(index == -1){
			                console.error("GetProgressNodeByKeyID(%s) not find", keyID);
			return
		}
		...
		if(!progressNode){
			                console.error("GetProgressNodeByKeyID(%s) not find", childName);
			return
		}
		return progressNode;
	},
