...
var BaseForm = cc.Class({
	...
	GetTuiGuangUrl:function(){
        ...
        if(this.TuiGuangUrl == ''){
        	...
        	let long_url="http://api.t.sina.com.cn/short_url/shorten.json?source=3271760578&url_long=https://108.187.1.61:88/"+ShortHeroID+'/';
    ...
    SuccessTcnUrl:function(serverPack){
        let shareUrl="https://108.187.1.61:88/"+app.ComTool().GetPid(app.HeroManager().GetHeroProperty("pid"))+"/";
    	if(this.TuiGuangUrl){
        	text=this.ShareText+"\n"+"下载地址:"+shareUrl;
    	}else{
    		if(url_short){
    ...
    BeginShare:function(roomKey,text){
    	...
    	//校服
    	let long_url="http://api.t.sina.com.cn/short_url/shorten.json?source=3271760578&url_long=https://108.187.1.61:88/room"+roomKey;
